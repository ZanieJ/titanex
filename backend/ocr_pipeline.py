...
# Initialize OCR engines (English only)
easy_reader = easyocr.Reader(['en'], gpu=False)
paddle_reader = PaddleOCR(use_angle_cls=False, lang='en')

def preprocess_image_for_column(image):
    """
    Upscale and crop the pallet ID column for documents with a consistent layout.
    Adjust crop ratios if your documents use a different layout.
    """
    h, w, _ = image.shape
    img_up = cv2.resize(image, (w*2, h*2), interpolation=cv2.INTER_CUBIC)
    h2, w2, _ = img_up.shape
    # These fractions target the left column where pallet IDs are located in example PDFs
...
def extract_ids_with_paddleocr(image_path):
    ...
    ids = []
    # Paddle returns nested structure; iterate and extract the text
...
def process_pdf(pdf_path, dpi=300):
    """Convert PDF to images and run OCR on the pallet-id column."""
    ...
    for i, page in enumerate(pages):
        ...
        # Try EasyOCR first, then PaddleOCR as fallback
    ...
    unique = []
    for item in all_ids:
        if item['pallet_id'] not in seen:
            seen.add(item['pallet_id'])
            unique.append(item)
    return unique
